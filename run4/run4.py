"""run4 — Old Jama vs New Jama name comparison.

The customer's xlsx has multiple sheets; the relevant ones are 'Old Jama' (~200
rows) and 'New Jama' (~400 rows). IDs and row positions don't line up across
the two sheets — same requirement may sit on different rows with different IDs.
Goal: produce a single comparison sheet that pairs each Old row with its
best-matching New row by Name, classifies the pair (identical or mismatch),
and lists the leftover New rows (the additions) at the bottom.

Pipeline:
  1. Read both sheets via extract_excel.
  2. Embed Names from each side (bge-small-en).
  3. Build all (old, new) candidate pairs ranked by cosine similarity. Greedy
     1:1 assignment from highest similarity downward — each Old claims at most
     one New, each New is claimed by at most one Old. Below a similarity floor
     the Old gets no candidate (status will be 'missing').
  4. LLM verifies each matched pair (binary identical/mismatch). Auto-identical
     shortcut for similarity >= 0.97.
  5. Output xlsx with two sections:
       Top: Old rows in Old's original order, with matched New on the right.
       Bottom: unclaimed New rows, left side blank.

Status vocabulary:
  identical   — names describe the same underlying requirement
  mismatch    — names paired by similarity but the LLM says they're different
  missing     — Old row has no good New candidate (requirement was removed/
                renamed beyond similarity threshold)
  unmatched   — New row not claimed by any Old (a new requirement)

Usage:
  python run4.py path/to/input.xlsx
  python run4.py path/to/input.xlsx --out path/to/output.xlsx --limit 30
"""
...
async def classify_pair(client: AsyncOpenAI, model: str,
    ...
    user = CLASSIFY_USER.format(
        old_id=old.jama_id, old_type=old.item_type or "", old_name=old.name[:1000],
        new_id=new.jama_id, new_type=new.item_type or "", new_name=new.name[:1000],
    )
    messages = [
...
def write_report(out_path: Path, olds: list[Row], news: list[Row],
                 ...
                 verdicts: dict[tuple[int, int], Verdict]) -> None:
    """Write the comparison xlsx."""
    wb = Workbook()
    ws = wb.active
    ws.title = "Old vs New"
...
def main() -> int:
    ...
    old_vecs = embed(embed_model, [r.name for r in olds], "old names")
    new_vecs = embed(embed_model, [r.name for r in news], "new names")
    sim = old_vecs @ new_vecs.T  # (|old|, |new|), already normalized
    ...
    # Build LLM jobs. Skip pairs at very high similarity — those names are
    # already essentially identical strings; the LLM call would be a waste.
    auto_identical: dict[tuple[int, int], Verdict] = {}
    jobs: list[tuple[Row, Row, float]] = []
    for old_idx, (new_idx, s) in matches.items():
        if s >= AUTO_IDENTICAL_THRESHOLD:
            auto_identical[(old_idx, new_idx)] = Verdict(
                old_index=old_idx, new_index=new_idx,
                status=ST_IDENTICAL, similarity=float(s),
                reasoning=f"(auto-identical) cosine similarity {s:.4f} ≥ {AUTO_IDENTICAL_THRESHOLD}",
            )
        else:
            jobs.append((olds[old_idx], news[new_idx], float(s)))
    print(f"auto-identical (sim ≥ {AUTO_IDENTICAL_THRESHOLD}): {len(auto_identical)}")
    ...
    write_report(out_path, olds, news, matches, verdicts)
